...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    </article><aside className="side-card project-summary"><strong>Project summary</strong><dl><div><dt>Status</dt><dd>{project.status}</dd></div><div><dt>Role</dt><dd>{project.role}</dd></div><div><dt>Project type</dt><dd>{project.type === 'SoftwareApplication' ? 'Software application' : 'Technical case study'}</dd></div><div><dt>Technology</dt><dd>{project.technologies.join(', ')}</dd></div><div><dt>Last updated</dt><dd>{new Date(project.updated).toLocaleDateString('en-GB', { dateStyle: 'long' })}</dd></div></dl>{project.demoUrl && <a className="button" href={project.demoUrl}>Open live demo</a>}{project.repositoryUrl && <a className="button secondary" href={project.repositoryUrl}>View source code</a>}<Link className="button" href={`/contact?enquiry=Software%20development&subject=${encodeURIComponent(project.title)}`}>Discuss a similar problem</Link></aside></div></section>
  </>;
}
